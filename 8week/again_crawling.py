...
fixed_file_path = '8week/prime_data_0727.json'
...
for rank in range(500):
    ...
    for item in fixed[str(rank)]:
        ...
        try:
            ...
            gpt_info = {"error":f"{e}"}
        if "urls" in gpt_info and (set(gpt_info["urls"])^set(item["urls"])) and list(set(gpt_info["urls"])-set(item["urls"])):
            gpt_info['old_urls'] = list(set(item["urls"])-set(gpt_info["urls"]))
            gpt_info['new_urls'] = list(set(gpt_info["urls"])-set(item["urls"]))
            lst.append(gpt_info)
    json_object[rank] = lst
    with open('8week/changed_data.json','w') as json_file:
